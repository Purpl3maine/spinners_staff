...
// Length of a rota shift in minutes, minus its unpaid break — used for the
// "scheduled hours" summary on the rota builder (planned hours, before
// anyone's actually clocked in). A end time of "00:00" means the shift runs
// to midnight (the most common overnight case for a pub — last orders/
// closing), so it's treated as the end of that calendar day rather than the
// very start of it.
function shiftDurationMinutes(start, end, breakMinutes) {
  const toMin = (t) => {
    const [h, m] = String(t).split(':').map(Number);
    return h * 60 + m;
  };
  const startMin = toMin(start);
  let endMin = end === '00:00' ? 1440 : toMin(end);
  if (endMin <= startMin) endMin += 1440; // safety net for any other overnight span
