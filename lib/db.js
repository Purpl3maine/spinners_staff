...
function seedData() {
  ...
  return {
    settings: { pubName: 'The Spinners', rolesMigrated: true },
    users,
    shifts,
    clockEvents,
    timeOffRequests,
...
// One-time upgrade for existing installs: the app used to have only
// 'manager' and 'staff' roles. Now there's an 'owner' tier above manager.
// Since every existing install only ever had one manager-level account (no
// in-app way to create more until this update), promote it to owner so
// nothing about who can do what changes for them. Guarded by a flag so it
// only ever runs once — later, intentionally-created 'manager' accounts
// must NOT get swept up in this on a future restart.
function migrateRoles(data) {
  data.settings = data.settings || {};
  if (data.settings.rolesMigrated) return false;
  (data.users || []).forEach((u) => {
    if (u.role === 'manager') u.role = 'owner';
  });
  data.settings.rolesMigrated = true;
  return true;
}

// Older data.json files predate rota templates, departments, manual rota
// ordering, open-shift claims, push notifications, and staff availability —
// default them in rather than requiring a migration flag. Runs on every
// load (fresh seed or existing file) so both paths stay consistent. Returns
// true if anything needed fixing up.
function normalizeRotaExtras(data) {
  let changed = false;
  if (!data.rotaTemplates) {
    data.rotaTemplates = [];
    changed = true;
  }
  if (!data.departments) {
  ...
  const missingOrder = (data.users || []).filter((u) => u.rotaSortOrder === undefined || u.rotaSortOrder === null);
  if (missingOrder.length) {
    // Give everyone a stable starting order (alphabetical) the first time
    // this runs; manual drag-reordering moves them from there.
    [...(data.users || [])].sort((a, b) => a.name.localeCompare(b.name)).forEach((u, i) => (u.rotaSortOrder = i));
    changed = true;
  }
  (data.users || []).forEach((u) => {
  ...
  // Payroll approval is a new gate on worked shifts — grandfather in every
  // clock-out recorded before this update as already approved, so existing
  // history isn't retroactively blocked from export. Only clock-outs from
  // here on default to unapproved and need a manager's review.
  (data.clockEvents || []).forEach((e) => {
    if (e.type === 'out' && e.approved === undefined) {
      e.approved = true;
      changed = true;
    }
  });
  data.settings = data.settings || {};
  if (!data.settings.vapid) {
    // Generated once per install and kept forever — the browser ties a
    // push subscription to this specific key, so rotating it would silently
    // break every subscription anyone had already set up.
    const keys = generateVapidKeys();
    data.settings.vapid = {
      publicKey: keys.publicKey,
      privateJwk: keys.privateJwk,
      subject: `https://${(process.env.RAILWAY_PUBLIC_DOMAIN || 'staff.spinnersdarwen.co.uk')}`,
    };
    changed = true;
  }
  return changed;
...
function load() {
  ...
  try {
    ...
    let needsSave = false;
    if (migrateRoles(cache)) {
      console.log('[roles] upgraded existing manager account(s) to the new "owner" role.');
      needsSave = true;
    }
    if (normalizeRotaExtras(cache)) needsSave = true;
    if (needsSave) save(cache);
  } catch (err) {
...
module.exports = { load, save, hashPassword, resetDemoData, DB_PATH, DATA_DIR };
