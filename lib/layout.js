...
const STAFF_NAV = [
  ...
  { href: '/staff/availability', label: 'Availability', icon: '✅' },
...
// bar), so they get their own personal self-service links too — the same
// clock, schedule, timesheet and time-off pages staff use, via /staff/*.
// "My" prefixes tell them apart from the similarly-named admin pages
// (e.g. "My timesheet" vs "Timesheets"). The owner doesn't get these: that
// account is treated as admin-only, same as elsewhere in the app.
const MANAGER_SELF_NAV = [
  { href: '/staff', label: 'Clock in/out', icon: '⏱' },
  { href: '/staff/schedule', label: 'My schedule', icon: '📅' },
  { href: '/staff/availability', label: 'My availability', icon: '✅' },
  { href: '/staff/timesheet', label: 'My timesheet', icon: '🧾' },
  { href: '/staff/time-off', label: 'My time off', icon: '🌴' },
];

function navFor(user) {
  if (user.role === 'owner') return MANAGER_NAV;
  if (user.role === 'manager') return [MANAGER_NAV[0], ...MANAGER_SELF_NAV, ...MANAGER_NAV.slice(1)];
  return STAFF_NAV;
}
